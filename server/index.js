...
async function getLiveSessionContextById(sessionId) {
  const result = await pool.query(
    `
    SELECT
      s.id AS session_id,
...
async function getRunningSessionContextByJoinCode(joinCode) {
  const result = await pool.query(
    `
    SELECT
      s.id AS session_id,
...
function sanitizeQuizQuestions(rawQuestions) {
  ...
  return rawQuestions.map((rawQuestion, questionIndex) => {
    ...
    if (answerMode === "single" && correctOptionsCount !== 1) {
      throw new Error(`Для одиночного выбора в вопросе №${questionIndex + 1} должен быть один правильный ответ.`);
    }

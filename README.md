...
### 4. Запуск только на ПК

Если приложение нужно открыть только на текущем компьютере:

1. Убедитесь, что в `.env` стоят локальные адреса:
...
### 5. Запуск на ПК и телефоне в одной сети

Если приложение должно открываться и на компьютере, и на телефоне:
...
2. Пропишите этот IP в `.env`:

```env
API_PORT=4000
REACT_APP_API_URL=http://10.115.160.76:4000
CORS_ORIGIN=http://localhost:3000,http://10.115.160.76:3000
```

Замените `10.115.160.76` на ваш текущий IP из локальной сети.

3. Запустите production-preview для телефона:
...
- собирает frontend
- перезапускает backend на `4000`
- отдает frontend и API с одного адреса без dev-overlay
...
```text
http://10.115.160.76:4000
```

Важно:

- телефон и ПК должны быть в одной Wi-Fi сети
- если IP компьютера изменился, обновите `REACT_APP_API_URL` и `CORS_ORIGIN` в `.env`
- для телефона используйте `npm run phone`, а не `npm run dev`
- если порт `4000` уже занят другим процессом, сначала освободите его или завершите старый сервер проекта
